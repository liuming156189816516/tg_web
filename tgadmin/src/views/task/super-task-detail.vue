<template>
  <div class="">
    ...
    <el-table ref="serveTable" :data="detailDataList" border height="660" v-loading="l_loading" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255,1)" style="width: 100%;" :header-cell-style="{ color: '#909399', textAlign: 'center' }" :cell-style="{ textAlign: 'center' }" @selection-change="handleSelectionChange" @row-click="rowSelectChange">
      ...
      <el-table-column prop="member_num" :label="$t('sys_q142')" minWidth="100" />
      <!-- <el-table-column prop="task_num" label="炸群数量" minWidth="100" /> -->
      <el-table-column prop="ad_account" :label="$t('sys_q135')" minWidth="100" />
      <el-table-column prop="state" :label="$t('sys_l059')" width="120">
    ...
    <el-dialog title="上传群组" center :visible.sync="groupModel" :close-on-click-modal="false" width="560px">
      <el-form size="small" :model="groupForm" :rules="groupRules" ref="groupForm" label-width="80px" class="demo-ruleForm">
        ...
        <el-form-item style="text-align:center;" label-width="0px">
          ...
          <el-button type="primary" :disabled="isLoading" @click="submitLink('groupForm')">确认</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  ...
  methods:{
    ...
    submitLink(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.isLoading=true;
          addmarketgroupinfotwo({invite_link:this.groupForm.group_link}).then(res=>{
            this.isLoading=false;
            if (res.code !=0 ) return;
            successTips(this)
            this.getDetailList(1);
